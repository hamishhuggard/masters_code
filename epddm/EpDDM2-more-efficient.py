...
class EpDDM: # Epistemic Drift Detection Method, or Expected Probability Drift Detection Method
    ...
    def __init__(self, drift_threshold=0.01, window_size=100):
        ...
        # Data storage
        self.buffer_x = [ 0 for i in range(window_size) ]
        self.buffer_y = [ 0 for i in range(window_size) ]
...
#         self.drop_next = [ True for i in range(tail_size) ]
        ...
        # Calcalations
        self.k = [0 for i in range(window_size)]
        self.thresholds = np.sqrt( 2 * np.log(drift_threshold) / np.arange(1, window_size+1) )
        self.warning = False
        self.drift = False
        self.n_samples = 0
...
#         print(probability_distribution, true_label)
        ...
        k = [ np.mean(buffer_x[:i+1]) for i in range(len(buffer_x)) ]
        self.k = k
        
        extremity = (k - self.thresholds)[:self.n_samples]
        
        warning_status = False
        drift_status = False
        
        try:
            most_extreme = max(extremity)
        except:
            most_extreme = 0
        
#         print(min_hoeffding, self.drift_threshold)
        if most_extreme > 0:
            # Then one of the k values exceeds its threshold
